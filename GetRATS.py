...
def parseTSENDatatoCSV(binData, OutFile): 
    ...
    with open(OutFile, mode='a+') as out_file:
        ...
        for y in range(int((len(binData)/30))):
            index = y*30        
            GPSTime = (struct.unpack_from('>I',binData,0+index)[0])
            ...
            RSSIval = (struct.unpack_from('>H',binData,24+index)[0])*-1
            SNRval = (struct.unpack_from('>h',binData,26+index)[0])
            Freqval = (struct.unpack_from('>h',binData,28+index)[0])

            header = [GPSTime,GPSLat, GPSLon, GPSAlt, TSEN_T, TSEN_P, TSEN_TP, TSEN_CalT, RSSIval, SNRval, Freqval]
            file_writer.writerow(header)
...
def TSENCalVal(T_counts):
    ...
    k = 62877.7030918
    #a0 = 0
    a1 = 0.000000364488854694
    a2 = -0.00000565461167644
    a3 = 0.000367513874002
    a4 = 0.000764859237036
    #a5 = 2.512381256e-08
  
    R = k*(T_counts-a)/(b-T_counts)
    #T=(1.0/(a0+a1*log(R)+a2*(log(R)**2)+a3*(log(R)**3)+a4*(log(R)**4)+a5*(log(R)**5)))-273.15
   # =(1/(a1*(LN(R)^3)+a2*(LN(R)^2)+a3*(LN(R)^1)+a4*(LN(R)^0))-273.15)

    T =(1.0/(a1*(log(R)**3)+a2*(log(R)**2)+a3*(log(R)**1)+a4*(log(R)**0))-273.15)
